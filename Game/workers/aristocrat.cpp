...
namespace Game {
Aristocrat::Aristocrat(const std::shared_ptr<iGameEventHandler>& eventhandler,
...
std::string Aristocrat::getType() const
{
    return "Aristocrat";
}

bool Aristocrat::canBePlacedOnTile(const std::shared_ptr<TileBase> &target) const
{
    return target->getOwner() == getOwner() and
            WorkerBase::canBePlacedOnTile(target);
}

void Aristocrat::doSpecialAction()
{
    //to do
}

const ResourceMapDouble Aristocrat::tileWorkAction()
{
