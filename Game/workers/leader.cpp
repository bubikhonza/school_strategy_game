...
namespace Game {
LeaderWorker::LeaderWorker(const std::shared_ptr<iGameEventHandler>& eventhandler,
...
std::string LeaderWorker::getType() const
{
    return "LeaderWorker";
}

bool LeaderWorker::canBePlacedOnTile(const std::shared_ptr<TileBase> &target) const
{
    return target->getOwner() == getOwner() and
            WorkerBase::canBePlacedOnTile(target);
}

void LeaderWorker::doSpecialAction()
{
    //to do
}

const ResourceMapDouble LeaderWorker::tileWorkAction()
{
