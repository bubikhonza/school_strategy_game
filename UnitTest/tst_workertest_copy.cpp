...
void WorkerTest::testGetType()
{
    ...
    QCOMPARE(ar->getType(), "Aristocrat");
    QCOMPARE(lw->getType(), "LeaderWorker");
    QCOMPARE(ew->getType(), "EngineerWorker");
}
...
void WorkerTest::testCanBePlacedOnTile()
{
    ...
    QCOMPARE(ew->canBePlacedOnTile(std::dynamic_pointer_cast<TileBase>(desert)), true);
    QCOMPARE(ew2->canBePlacedOnTile(std::dynamic_pointer_cast<TileBase>(desert)), false);
    QCOMPARE(ew2->canBePlacedOnTile(std::dynamic_pointer_cast<TileBase>(desert2)), true);
}
